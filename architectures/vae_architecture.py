...
class Encoder(tf.keras.Model):
    ...
    def __init__(self, stride, kernel_size, padding, starting_filters, latent_dim, n_conv_layers, activation="relu"):
        ...
        self.conv_layers = [layers.Conv2D(starting_filters*i,
                                        kernel_size,
                                        strides=stride,
                                        padding=padding,
        ...
        self.mean_dense_layer = layers.Dense(latent_dim, activation=activation)
        self.std_dense_layer = layers.Dense(latent_dim, activation=activation)
...
class Decoder(tf.keras.Model):
    ...
    def __init__(self, stride, kernel_size, padding, starting_filters, n_conv_layers, activation="relu"):
        ...
        self.conv_transpose_layers = [layers.Conv2DTranspose(starting_filters*i,
                                        kernel_size,
                                        strides=stride,
                                        padding=padding,
                                        data_format="channels_last",
    ...
    def call(self, inputs):
        ...
        if self.dense_layer is None:
            self.dense_layer = layers.Dense(compressed_shape[1]*compressed_shape[2]*compressed_shape[3], activation=self.activation)
